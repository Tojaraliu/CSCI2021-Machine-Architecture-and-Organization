...
int addOK(int x, int y) {
    int sum = x + y;
    // Overflow iff signs are the same and the sign of sum is different.
    return !(~((x >> 31) ^ (y >> 31)) & ((y >> 31) ^ (sum >> 31)));
}
/*
...
int isUppercaseLetter(int x) {
    int uplimit = x + (~0x41 + 1); //should >=0
    int lowlimit = x + (~0x5A + 1); // should <=0
    return ~(uplimit>>31) & ((((lowlimit>>31) & 1) | (!lowlimit)));
}
/*
...
unsigned float_times_four(unsigned uf) {
    ...
    if (exp != 0)
        ...
        if (frac >= 1<<21) {
            frac = (frac << 1) & 0x007FFFFF; // I dont understand why and how this line works...
            exp += 2;
        }
        else 
            frac = (frac << 2);
